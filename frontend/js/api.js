...
const LOCAL_STORAGE_ADMISSIONS_KEY = 'nitt_admissions_data';
const LOCAL_STORAGE_CONTACT_KEY = 'nitt_contact_data';
...
const NITT_API = {
  ...
  async submitAdmission(formData) {
    ...
    try {
      ...
      const fallbackId = `NITT-${new Date().getFullYear()}-${Math.floor(1000 + Math.random() * 9000)}`;
  ...
  async submitContact(formData) {
    ...
    try {
      ...
      return {
        success: true,
        message: 'Thank you for contacting NITT. Your message has been received.',
        data: fallbackRecord,
        storage: 'browser_fallback'
      };
    }
