...
function initFormValidationAndSubmission() {
  ...
  form.addEventListener('submit', async (e) => {
    ...
    submitBtn.innerHTML = `<span class="spinner-border spinner-border-sm me-2"></span> Submitting to NITT Portal...`;

    try {
      const response = await NITT_API.submitAdmission({
        fullName,
        email,
        phone,
        course,
      ...
      if (modalRef) modalRef.textContent = appData.application_id || 'NITT-2026-REG';
...
function initAdminViewer() {
  ...
  trigger.addEventListener('click', async () => {
    ...
    listContainer.innerHTML = `<div class="text-center py-4"><div class="spinner-border text-primary"></div><p class="mt-2 small text-muted">Fetching records from NITT system...</p></div>`;

    const modal = new bootstrap.Modal(document.getElementById('adminViewerModal'));
    modal.show();

    try {
      const admissions = await NITT_API.getAdmissions();
        ...
        <div class="table-responsive">
          <table class="table table-hover align-middle">
            ...
            <tbody style="font-size: 0.9rem;">
              ${admissions.map(adm => `
                <tr>
                  <td><span class="badge bg-navy text-white">${adm.application_id || 'NITT-2026-ADM'}</span></td>
